...
def sort_rows(rows: List[ReleaseRow], various_policy: str) -> List[ReleaseRow]:
...
def generate_txt_lines(rows: List[ReleaseRow], dividers: bool = False,
                       align: bool = False, show_country: bool = False) -> List[str]:
  ...
  def formatted_line(r: ReleaseRow) -> str:
    ...
    if align:
      return f"{r.artist_display.ljust(artist_width)} | {r.title.ljust(title_width)}{year_str}{label_part}{country_part}".rstrip()
    return f"{r.artist_display} — {r.title}{year_str}{label_part}{country_part}".rstrip()
